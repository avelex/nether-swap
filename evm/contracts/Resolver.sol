...
import {IResolverExample} from "1inch-cross-chain-swap/contracts/interfaces/IResolverExample.sol";
...
import {TimelocksLib, Timelocks} from "1inch-cross-chain-swap/contracts/libraries/TimelocksLib.sol";
import {AddressLib, Address} from "solidity-utils/contracts/libraries/AddressLib.sol";
import {IEscrow} from "1inch-cross-chain-swap/contracts/interfaces/IEscrow.sol";
import {ImmutablesLib} from "1inch-cross-chain-swap/contracts/libraries/ImmutablesLib.sol";
...
contract Resolver is Ownable {
    ...
    function deployDst(IBaseEscrow.Immutables calldata dstImmutables, uint256 srcCancellationTimestamp) external onlyOwner payable {
        address token = AddressLib.get(dstImmutables.token);
        IERC20(token).approve(address(_FACTORY), type(uint256).max);
